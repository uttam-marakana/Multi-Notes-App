...
import { trashBoard } from "../../utils/trashStorage";
...
export default function BoardManager({
  ...
  const { boards, toggleBoardPin, loading, duplicateBoard } = useBoard();
  ...
  const executeDelete = async (boardId) => {
    ...
    try {
      // UI-only trash (restore via Trash page). Firestore data is not permanently deleted here.
      trashBoard(boardId);
    ...
    <div className="board-manager glass-container">
      ...
      {loading && (
        <div className="loading-state">
          <SkeletonLoader variant="board" count={6} />
        </div>
      )}
